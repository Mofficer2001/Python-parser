...
class FB_IMp_S(Sensor):
    ...
    def to_hmi(self, x: int, y: int) -> str:
        hmi_block = (
            f'    <object access-modifier="private" name="{getattr(self, self.Name.key)}" display-name="{getattr(self, self.Name.key)}" uuid="{self.pk}" base-type="FB_IMp_S" base-type-id="2c2e050d-b19a-4f42-9f33-678792612a71" ver="5" description="" cardinal="1">\n'
