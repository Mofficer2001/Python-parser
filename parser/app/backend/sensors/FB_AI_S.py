...
class FB_AI_S(Sensor):
    ...
    def to_hmi(self, x: int, y: int) -> str:
        hmi_block = (
            f'    <object access-modifier="private" name="{getattr(self, self.Name.key)}" display-name="{getattr(self, self.Name.key)}" uuid="{self.pk}" base-type="FB_AI_S" base-type-id="8a805344-fee1-48ec-b249-f2c9aa9ac24c" ver="5" description="" cardinal="1">\n'
