...
class FB_OZK_KD_S(Sensor):
    ...
    def to_hmi(self, x: int, y: int) -> str:
        hmi_block = (
            f'    <object access-modifier="private" name="{getattr(self, self.Name.key)}" display-name="{getattr(self, self.Name.key)}" uuid="{self.pk}" base-type="FB_OZK_KD_S" base-type-id="b7cfe669-3295-4243-96af-503ef0e966ca" ver="5" description="" cardinal="1">\n'
            ...
            f'        <designed target="X" value="{x+20}" ver="4"/>\n'
