...
class FB_IMp_MON_S(Sensor):
    ...
    def to_hmi(self, x: int, y: int) -> str:
        hmi_block = (
            f'    <object access-modifier="private" name="{getattr(self, self.Name.key)}" display-name="{getattr(self, self.Name.key)}" uuid="{self.pk}" base-type="FB_IMp_MON_S" base-type-id="48b0260d-fe91-42bd-b052-a821fc03d5c0" ver="5" description="" cardinal="1">\n'
            ...
            f'        <designed target="X" value="{x-12}" ver="4"/>\n'
            f'        <designed target="Y" value="{y+165}" ver="4"/>\n'
