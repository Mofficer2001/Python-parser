...
class FB_UPP_S(Sensor):
    ...
    def to_hmi(self, x: int, y: int) -> str:
        hmi_block = (
            f'    <object access-modifier="private" name="{getattr(self, self.Name.key)}" display-name="{getattr(self, self.Name.key)}" uuid="{self.pk}" base-type="FB_UPP_S" base-type-id="9674d29e-bfdb-49fc-9ac6-cfe0293a7dcc" ver="5" description="" cardinal="1">\n'
            ...
            f'        <designed target="X" value="{x+155}" ver="4"/>\n'
            f'        <designed target="Y" value="{y+30}" ver="4"/>\n'
